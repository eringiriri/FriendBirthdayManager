...
| birthday | TEXT | NOT NULL | 誕生日 (YYYY-MM-DD形式) |
...
### 通知タイミング
- **毎日 9:00 AM** にチェック（設定可能にする場合は設定画面に追加）
- 各友人について「今日が通知日か」を判定
...
- 11月17日 9:00 AM → 「山田太郎さん (11/20) まであと3日」
- 11月18日 9:00 AM → 「山田太郎さん (11/20) まであと2日」
- 11月19日 9:00 AM → 「山田太郎さん (11/20) まであと1日」
- 11月20日 9:00 AM → 「今日は山田太郎さんの誕生日です！」

重複防止のため notification_history テーブルに記録
```
...
public class Friend
{
    ...
    // ヘルパーメソッド
    public int DaysUntilBirthday(DateTime today) { }
...
2,佐藤花子,1998-11-25,"hanako,はなこ",大学の先輩,,1,
```
- UTF-8 BOM付き
- ヘッダー行必須
- NULL値は空欄
...
### 設計上の未決定事項（後で決定可能）
- [ ] 通知時刻（現在9:00 AM）を設定可能にするか？
- [ ] 通知の繰り返し（同日に複数回）は必要か？
- [ ] 年齢表示機能は必要か？
- [ ] 誕生日の年を省略可能にするか？（毎年繰り返す前提）
